...
while 1
    ...
    while(GETSTIMULUSNAME)
        ...
        prompt = ['Which stimulus would you like to run? Options:q,step(amp,dur),stepLoop(amp,dur,reps), LEDstim(stim,isi,reps),\n' ...
            'LEDstim_currInject(stim,isi,inject,offset,reps), noCurrent(dur), currentSteps(lowerAmp,upperAmp,(opt:numSteps,stimDur,isi)), \n' ...
            'LEDstim_offPeriod(stim,isi,reps,offPeriodStartSec,offPeriodDurMin), makeInjectionWaveform(InjpA,BaseDur,PulseDur,PostDur), \n' ...
